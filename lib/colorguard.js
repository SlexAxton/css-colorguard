...
function renderConflictLine(data) {
  return '_match_ (_hex_) [line: _lines_]'
    ...
    .replace('_lines_', data.map(function (info) {
      return info.declaration.position.start.line + ':' + getColumnPositionRelativeToLine(info, 'start');
    }).join(', '));
}

function renderOriginalPosition(data) {
  // data is not always array (sometimes we don't provide map file)
  var result = data && data.map(function (position) {
...
function getOriginalPosition (info) {
  return smc && smc.originalPositionFor({
    ...
    for (var j = i + 1; j < colorLen; j += 1) {
      ...
      infoBlock = {
        colors: [{
          rgb: colorNames[i],
          lines: c1,
          originalLines: map && c1.map(getOriginalPosition)
        }, {
          rgb: colorNames[j],
          lines: c2,
          originalLines: map && c2.map(getOriginalPosition)
        }],
        distance: diffAmount
      };
      ...
      if (diffAmount < threshold && !whitelistHash[getWhitelistHashKey([colorNames[i], colorNames[j]])]) {
        infoBlock.message = '_1_ _1original_ is too close (_diffAmount_) to _2_ _2original_'
          .replace('_1_', renderConflictLine(c1))
          .replace('_2_', renderConflictLine(c2))
          .replace('_1original_', renderOriginalPosition(infoBlock.colors[0].originalLines))
          .replace('_2original_', renderOriginalPosition(infoBlock.colors[1].originalLines))
