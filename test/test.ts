...
var tests = [
  ...
  },
  {
    ...
    warnings: 1,
    warningsColors: [{ secondColor: '#fff', firstColor: 'white' }],
  },
  {
    message: 'identical but different length hexes with default setting',
    fixture: '.foo { color: #888; } .bar { color: #888888; }',
    warnings: 1,
    warningsColors: [{ secondColor: '#888888', firstColor: '#888' }],
  },
  {
    message:
      'identical but different length hexes with `allowEquivalentNotation`',
    fixture: '.foo { color: #888; } .bar { color: #888888; }',
    warnings: 0,
  ...
  tape(test.message, function (t) {
    ...
    check(test.fixture, test.options, function (css, warnings) {
      ...
      t.equal(
        warnings.length,
        test.warnings,
        'should send the correct number of warnings'
      );
      var warningsColors = warnings.map(function (warning) {
        return {
          secondColor: warning.secondColor,
          firstColor: warning.firstColor,
        };
      });
      t.deepEqual(
        warningsColors,
        test.warningsColors || [],
        'should identify the correct colors'
      );
    });
  });
});
